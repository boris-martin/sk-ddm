...
def make_local_solve(
    ...
    def local_solve(gloc: np.ndarray) -> np.ndarray:
        ...
        if rhs.shape[0] != total_size:
            raise ValueError(f"RHS size mismatch: expected {total_size}, got {rhs.shape[0]}")
...
if ndom == 1:
    ...
    gamma_facets = mesh_helpers.findFacetsGamma(
        gamma_tags, gmshToSK, mesh_helpers.buildFacetDict(mesh)
    )
    if len(gamma_facets) == 0:
        ...
        local_source = skfem.asm(plane_wave.plane_wave, gamma_basis, k=k, theta=theta).astype(
            np.complex128
        )
...
for idom in range(1, ndom + 1):
    ...
    for j, sigma_tag in sigma_tags.items():
        ...
        for edge in subdomain.all_sigma_facets[j]:
            for node in mesh.facets[:, edge]:
                nodeSet.update({node})
        function_space = skfem.FacetBasis(
            mesh, ElementTriP1(), facets=subdomain.all_sigma_facets[j]
        )
        for node in nodeSet:
            nodesToJset[subdomain.SKToGmsh[node]].add(j)
        projector, sk_to_g = scipy_helpers.restriction_matrix(
            mesh.nvertices, list(nodeSet), subdomain.SKToGmsh, subdomain.gmshToSK
        )
    ...
    gamma_facets = mesh_helpers.findFacetsGamma(
        gamma_tags, subdomain.gmshToSK, subdomain.facets_dict
    )
    print("gamma_facets has size ", len(gamma_facets))

    mats: list[list[csr_matrix | None]] = [
        [None for _ in range(num_interface_fields + 1)] for _ in range(num_interface_fields + 1)
    ]
    mats[0][0] = skfem.asm(helmholtz, skfem.Basis(mesh, ElementTriP1()), k=k)
    if len(gamma_facets) > 0:
        mats[0][0] += skfem.asm(
            absorbing, FacetBasis(mesh, ElementTriP1(), facets=gamma_facets), k=k
        )

    subdomain.set_neuman_mat(
        scipy.sparse.csr_matrix(mats[0][0]) if mats[0][0] is not None else None
    )  # Deep copy
    ...
    for evidx in range(nev):
        ...
        g_continuous[bnd_dofs] += scipy.sparse.linalg.spsolve(
            sigma_mass_restricted, (A_s @ x)[bnd_dofs]
        )
    ...
    b_substructured = scipy.sparse.linalg.spsolve(subdomain.get_problem_mat(), full_rhs)[
        mesh.nvertices :
    ]
...
all_coarse_spaces = [
    subdomains[i].gen_global_coarse_space_contrib(total_g_size, istart[i]) for i in range(ndom)
]
...
for i, cs in enumerate(all_coarse_spaces):
    if cs is not None:
        start_col = sum(
            all_coarse_spaces[j].shape[1] for j in range(i) if all_coarse_spaces[j] is not None
        )
...
m_inv_delta = scipy.sparse.linalg.spsolve(
    full_mass, delta_kernel.reshape(total_g_size, -1)
).reshape(total_g_size, -1)
print(
    "Is it a kernel ? Ax has norm ",
    np.linalg.norm(ddm_op.A @ m_inv_delta),
    " and x has norm ",
    np.linalg.norm(m_inv_delta),
)
m_s_inv_delta = full_mass @ scipy.sparse.linalg.spsolve(
    full_s_mass, delta_kernel.reshape(total_g_size, -1)
).reshape(total_g_size, -1)
print("Shape of msinv delta: ", m_s_inv_delta.shape)
print(
    "Is it a kernel for the adjoint? A^*x has norm ",
    np.linalg.norm(np.conj(ddm_dense.T) @ m_s_inv_delta),
    " and x has norm ",
    np.linalg.norm(m_s_inv_delta),
)
...
scipy.sparse.linalg.gmres(
    ddm_op.A, rhs, rtol=1e-6, callback=lambda r: residual.append(r), maxiter=1000
)
counter = 0
residual_prec = []
scipy.sparse.linalg.gmres(
    ddm_op.A @ precond_ide, rhs, rtol=1e-6, callback=lambda r: residual_prec.append(r), maxiter=1000
)
...
u, s, vt = svd(
    ddm_op.A @ spla.spsolve(full_mass, np.eye(total_g_size, dtype=np.complex128)),
    full_matrices=False,
)
# u, s, vt = svd(ddm_op.A @ np.eye(total_g_size, dtype=np.complex128), full_matrices=False)
...
mism = scipy.sparse.linalg.spsolve(
    full_mass, full_s_mass @ scipy.sparse.linalg.spsolve(full_mass, full_mass @ rhs)
)
...
rhs = np.random.normal(size=(total_g_size,)) + 1j * np.random.normal(size=(total_g_size,))
x_rand, info_rand = scipy.sparse.linalg.gmres(
    ddm_op.A, rhs, rtol=1e-6, callback=lambda r: print("GMRES residual (rand RHS): ", r)
)
print("M-orthogonalizing the RHS against the kernel")
y = scipy.sparse.linalg.spsolve(
    np.conjugate(ker.T) @ full_mass @ ker, np.conjugate(ker.T) @ (full_mass @ rhs)
)
rhs = rhs - ker @ y
print("Checking orthogonality after M-orthogonalization: ", np.conjugate(ker.T @ full_mass) @ rhs)
x_rand_ortho, info_rand_ortho = scipy.sparse.linalg.gmres(
    ddm_op.A, rhs, rtol=1e-6, callback=lambda r: print("GMRES residual (rand ortho RHS): ", r)
)
